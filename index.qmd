---
title: " Detection of Knee Pathologies from MRI Using Deep Learning Models (CNNs)"
...
Slides: [Slides.html](presentation_slides.html){target="_blank"}
...
Sentence rephrasing and words replacement and overall text clarity enhancement has been implemented using an AI-based engine[@AI_tool]. All computational analyses were implemented using Python (Python Software Foundation, 2024).[@python].
...
#| out-width: "50%"
#| fig-align: "center"
knitr::include_graphics("figures/mri_slices.png")
```
</center>
...
We train our network on mini-batch inputs of MRI volumes. We use Focal Loss instead of binary cross entropy loss to handle the class imbalanced problem. We use Adam optimizer with weight decay. We update the learning rate every few epochs with a learning rate scheduler. We train for 15 epochs and record the loss.
...
The training loss decreased over the 15 epochs. The network showed fast initial learning followed by slow convergence towards the end.
...
For all 3 tasks, ROC plots have been produced to assess the models accuracy for classification. The discrimination power is very high for all tasks:

- ACL: 0.827
- Meniscus: 0.746
- Abnormal: 0.833

The model seems to achieve consistent accuracy throughout the classification tasks; particularly in its ability to identify ACL tears and other abnormalities.
...
<center>
![Figure 6: ROC Curves](Images/Results/ROC curve.png)
</center>
...
For evaluation of the classification performance of the model on each of the three tasks (i.e., ACL tears, meniscus tears and abnormality detection), confusion matrices have been calculated for each of the individual tasks. Each confusion matrix includes the number of true negatives (TN), false positives (FP), false negatives (FN) and true positives (TP).
...
<center>
![Figure 7: Confusion Matrix](Images/Results/Confusion new.png)

</center>

The confusion matrices produced strong data that showed better sensitivity and specificity for this model than previously established models.

The TP and TN in ACL classification showed almost equal results with 41 and 51 respectively, although there were 15 FP and 13 FN.

The meniscus classifier is somewhat accurate but tends to fall in the range of overestimating cases of meniscus tear with 19 FP.

The abnormality classifier shows much greater TP number (85) than TN (17) and seems more sensitive.
...
To evaluate the confidence of the model in positive and negative predictions, probability distribution charts were generated. From the figures, it is hoped that the positive predictions would center around values close to 1 and the negative ones around 0.
...
<center>
![Figure 8: ACL Probability Distribution](Images/Results/ACL Probability new.png)

</center>

<center>
![Figure 9: Meniscus Probability Distribution](Images/Results/Meniscus new.png)
</center>

<center>
![Figure 10: Abnormal Probability Distribution](Images/Results/Abnormal new.png)
</center>


-**ACL:**

From the ACL probability distribution chart we can see that there is a fairly good separation between positive and negative cases, but there is still some overlap.

-**Meniscus:**

The big challenge there is that there is a huge overlap between the two classes (positive and negative samples) and it is really hard to distinguish between them due to the limited structural differences.

-**Abnormal:** 

The model has Strong Confidence in its Predictions and Separation. 


**3.4.5 Plane Attention Weights**

These are plane-attention weights for the predictions
...
<center>
![Figure 11: Plane Attention Weights](Images/Results/Plane new.png)
</center>


Sagittal Has the Greatest Importance (~0.36): The model weight shows that the data from the Axial plane is most important for the model to be able to diagnose the given disease, likely because the Axial image plane contains the most relevant evidence or distinctive features for that disease.

Axial Plane Is the Second Most Important (~0.34): The importance measure for the Sagittal plane is actually very close to that of the Axial plane. This means that both body planes contain important diagnostic information.

Coronal Has the Least Importance (~0.30): The lowest importance measure was the Coronal plane, indicating that the features in this image plane are likely to be redundant or less important than the other two image planes.
...
Performance metrics of the trained network on the validation dataset were calculated after training of the network. Binary output values were created by first obtaining the output probabilities using the sigmoid activation function from the trained network.
...
The 3D CNN-based model was validated using a validation dataset to determine its sensitivity, specificity, accuracy, AUC and F1-score scores for the three different classification tasks of differentiating ACL tears from non-ACL tears, meniscus tears from non-meniscus tears, and abnormal knee structures from normal knee structures.

-**ACL:**
For the task of ACL classification, a balanced performance was achieved with a sensitivity of 0.759 and a specificity of 0.773. This is accompanied by high accuracy of 0.767 and AUC value of 0.827 reflecting good discriminative capability. Additionally, the F1-score of 0.745 denotes well-balanced precision and recall.

-**Meniscus:**
The model shows moderate performance on meniscus classification with sensitivity 0.731 and specificity 0.721, and accuracy 0.725. The corresponding AUC is 0.746. While it performs better than the other two tasks, it still shows relatively lower performance compared to them. The F1-score of 0.697 for meniscus tear detection indicates it to be a more challenging task due to some of the very small variations in appearance, which the model has difficulty in detecting and accordingly classifying.

-**Abnormal:**
For identifying abnormal cases, the model reaches a high sensitivity of 0.895. However, the achieved specificity of 0.680 is somewhat below what would be desirable. On the other hand, both accuracy of 0.850 and AUC of 0.833 indicate good results. The highest F1-score of 0.904 for an individual class underscores the good balance between precision and recall.

The model performs well on all tasks and was particularly effective for identifying abnormalities.

## Conclusion

In this project, a three-class classification model has been trained to identify the presence of three knee pathologies (tears of the ACL, meniscal tears and generalised knee abnormality) from volumetric multi-plane MRI data. Each knee pathology has been treated as a separate binary problem. Results have been obtained on the MRNet data with a clinically meaningful performance that translates into a model that is well-calibrated and does not sacrifice one type of error for the other. The best results for the problem of detecting a torn ACL have been an AUC-ROC score of 0.827, sensitivity of 0.759, specificity of 0.773, accuracy of 0.767 and an F1 score of 0.745. The hardest problem of the three to solve has been the detection of meniscal tears for which the best results have an AUC-ROC of 0.746, sensitivity of 0.731, specificity of 0.721, accuracy of 0.725 and an F1 score of 0.697. This is consistent with the hard work of a radiologist to confidently detect such lesions that are based on very subtle changes to the normal meniscal structure that can occur in multiple configurations. In contrast, the problem of generalised abnormality detection has obtained the best results of the study with an AUC-ROC of 0.833, sensitivity of 0.895, accuracy of 0.850 and an F1 score of 0.904. The trained loss plot for the model over the epochs indicates that good generalization to the validation set has been obtained before the fourteenths epoch to avoid overfitting to the training data distribution. Importantly, the performance of the three learnable plane-attention mechanisms has revealed that the converged weights have assigned a weight of 0.355 to the sagittal plane, 0.344 to the axial plane, and 0.305 to the coronal plane. This is consistent with current radiological practice whereby the sagittal plane is often the preferred view for ligamentous injuries, and the axial view provides cross-sectional views of the knee joint. The coronal planes are important additional views but they provide meaningfully less independently unique information. These results and approaches are consistent with deep learning-based MRI triage being viable and potentially useful in the short-term for workload reduction and potentially in reducing inter- and intra-observer variability in slice-by-slice image diagnoses.

Despite the progress made, there are still several limitations and challenges faced in the current project. Firstly, the amount of training data utilised is insufficient for practical clinical application and reliance on a single dataset is a major limitation. There is a significant class-imbalance in the labels whereby there are approximately four times as many ACL-negative cases as there are ACL-positive cases, which posed a challenge during training. Although utilising a class-weighted loss function improved model performance (as shown in the confusion matrices in Appendix B), it still did not generalise well to the majority of the negative cases. Future work would seek to address these limitations by accessing larger multi-centre datasets consisting of MRI examinations. Alternative training strategies such as semi-supervised and self-supervised training methods that can effectively train models on large amounts of unlabelled images would also be explored. There is also scope to investigate the use of transformer-based volumetric architectures that can capture long-range spatial relationships within knee MRI volumes, going beyond local spatial relationships captured by conventional convolutional kernels. Most importantly, future work would focus on prospective validation of the model on a large, independently collected dataset with a set of expertly diagnosed and annotated radiologist interpretations.
