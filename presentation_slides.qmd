...
Class Weights: [4.43, 1.85, 0.24]

It indicates strong imbalance (ACL is rare, Abnormal is mostly positive)

- **Focal Loss** — Focuses on hard, misclassified examples
  - Formula: FL(p) = −α(1−p)^γ · log(p)
  - Focal Loss helps the model learn better from minority classes
- **Data Augmentation** — Random flips, rotations, intensity scaling
- **Youden's Index** — Maximizes (Sensitivity + Specificity − 1)
  - Best threshold for ACL: 0.356
  - Best threshold for Meniscus: 0.400
  - Best threshold for Abnormal: 0.497
...
![Training Loss Curve](Images/Results/Training Loss.png){width="100%"}
...
- The curve shows smooth convergence without major oscillations.
- No clear over-fitting trend observed (no divergence between training stability and expected validation behavior).
...
![ROC_Curves](Images/Results/ROC_curves.png){width="100%"}
...
- ROC curves show strong performance across all tasks, with all models performing significantly better     than random classification (diagonal line).
- Overall, the model performs best for disease screening (abnormality detection) and moderately well for   specific pathology classification (ACL, Meniscus).
...
![Plane Attention Weights](Images/Results/Plane new.png){width="100%"}
...
- Model assigns different importance to each MRI plane  
- Higher weight = more influence on final prediction  
- Shows model is learning **which view is more informative**  
- Not treating sagittal, coronal, and axial equally  
- Provides interpretability into model decision-making  
